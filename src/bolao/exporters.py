...
def live_podium_html(live_scores: list[dict], status_label: str = "Aprovado", date_label: str | None = None) -> str:
    if not date_label:
        from .utils import now_iso
        date_label = now_iso()
    ...
    for idx, score in enumerate(top, start=1):
        ...
        cards.append(
            ...
            <div class="podium-place {place_class}">
              ...
              <div class="points">{score['total']} <span class="pts-label">pts</span></div>
              <div class="details">
                <span class="detail-item">{score['tie_breaker']}</span>
              </div>
            </div>
            """
        )
...
<meta charset="utf-8">
<meta name="viewport" content="width=device-width, initial-scale=1.0">
<title>Pódio Jogo a Jogo — Bolão da Cabine do Glória</title>
<style>
* {{
  box-sizing: border-box;
}}
...
<div class="wrap">
  <div class="trophy-icon">🏆</div>
  <h1>Bolão Cabine do Glória</h1>
  <div class="subtitle">Copa do Mundo 2026 · Pódio Jogo a Jogo</div>
  <div class="podium-container">
    {''.join(cards)}
  </div>
  <div class="footer">
