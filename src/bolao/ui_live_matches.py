...
def is_match_open_for_prediction(match: Any, now: str | None = None) -> bool:
    ...
    # 3. Parse and normalize for comparison
    if now is None:
        now_dt = datetime.now()
    else:
        try:
    ...
    try:
        lock_dt = datetime.fromisoformat(lock_at_str)
    except Exception:
        return False
        
    # Handle naive vs aware
    if now_dt.tzinfo is not None and lock_dt.tzinfo is None:
        now_dt = now_dt.replace(tzinfo=None)
    elif now_dt.tzinfo is None and lock_dt.tzinfo is not None:
        now_dt = now_dt.astimezone(lock_dt.tzinfo)
            ...
            for m in open_matches:
                ...
                with st.container(border=True):
                    ...
                    with c_btn:
                        ...
                        if st.button("💾 Salvar", key=f"save_btn_{m.match_id}", type="primary", width="stretch"):
...
def render_match_center() -> None:
    ...
    if is_open:
        ...
        # WhatsApp text do próprio usuário
        if user_pred:
            st.markdown("---")
        ...
        # Quando o jogo está bloqueado, exibe tudo conforme a privacidade
